public class lista5ex12e {
    ...
    static double [][] conta(double[][]tabela){
        double[][]guarda = new double[1][2];  //guarda as respostas
        double somaMarco = 0;
        double somaMaio = 0;


        for(int i = MARCO; i <= MARCO; i++){
            for(int j = 0; j < tabela.length; j++){
                somaMarco = somaMarco + tabela[j][i];
            }
        }

        guarda[0][0] = somaMarco;

        for(int i = MAIO; i <= MAIO; i++){
            for(int j = 0; j < tabela.length; j++){
                somaMaio = somaMaio + tabela[j][i];
            }
        }
    ...
    public static void main(String[]args){
        ...
        System.out.println("Vendas de Março: "+conta(tabela)[0][0]);
        System.out.println("Vendas de Maio: "+conta(tabela)[0][1]);
    }
}
